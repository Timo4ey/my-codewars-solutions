...
def cakes(recipe: dict[str, int], available: dict[str, int]):
    cooked = 0
    is_cooking: bool = len(recipe) <= len(available)

    while is_cooking:
        for name, count in recipe.items():
            av_count = available.get(name)
            if not av_count or av_count - count < 0:
                is_cooking = False
                break
            else:
                available[name] = av_count - count
        else:
            cooked += 1
